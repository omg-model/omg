function [ OMG_fcns ] = OMG_functions ( )
    ...
    OMG_fcns.AndAcc            = @AndAcc;
    OMG_fcns.solve_anderson    = @solve_anderson;
...
function [ TRACERS, ATM ] = solve_anderson (TRACERS,ATM,OCEAN,ECC,parameters,functions,forcings,bioinf,diags_on)
    ...
    AAstart = 0; 
    g=1;
    xscale=[reshape(repmat(sum(TRACERS.*parameters.ocn_pars.M)./sum(parameters.ocn_pars.M),parameters.ocn_pars.nb,1),[],1);ATM'];
    %xscale=ones(size(xinit,1),1);
    [xsol,iter,res_hist] = AndAcc(g,xinit./xscale,mMax,itmax,atol,rtol,droptol,beta,AAstart,xscale,pars_in);
    xsol=xsol.*xscale;
    ...
    TRACERS = reshape(xsol(1:parameters.ocn_pars.nb*parameters.gen_pars.n_tracers) , parameters.ocn_pars.nb , parameters.gen_pars.n_tracers );
    if iter<itmax
        disp(['solution found after ' num2str(iter) ' calls to the model function'])
    elseif iter==itmax
         disp(['maxmium iterations: termination criterion not met after ' num2str(itmax) ' iterations'])
         disp(['this is not necessarily bad!'])
    end

    dlmwrite([parameters.gen_pars.OMG_output_dir,'/anderson_solver_history.txt'],res_hist,'\t');
...
function [x,iter,res_hist] = AndAcc(g,x,mMax,itmax,atol,rtol,droptol,beta,AAstart,xscale,pars_in)
...
for iter = 0:itmax
    ...
    %gval = g(x); JDW
    gval = calc_F(x.*xscale,pars_in)./xscale;
    fval = gval - x;
    tmp(iter+1)=gval(end).*xscale(end);
    res_norm = norm(fval);
    ...
    %if iter == 0, tol = max(atol,rtol*res_norm); end % original tol
    if iter == 0, tol = atol+rtol*res_norm; end % same tol as newton solvers
