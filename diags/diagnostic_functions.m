function [ diag_fcns ] = diagnostics ( )
    ...
    diag_fcns.open_matobj    = @open_matobj;
...
%%
function [ matObj ] = open_matobj(path)
% find number of completed years in output file 'matObj'
% nyr is number of non-empty PO4 cells

    matObj = matfile([path '/OUTPUT.mat']);
end


%%
function [ i_lastyr ] = last_year(matObj)
% find number of completed years in output file 'matObj'
% nyr is number of non-empty PO4 cells

    i_lastyr = find(~cellfun(@isempty,matObj.PO4),1,'last');
end
