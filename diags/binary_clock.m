...
for i=1:eco_pars.ngenes
    bin_gene = dec2bin(GENOME(iextant,i),64); % convert double value to binary string
    str=[str bin_gene];
end
% convert to numeric values (1/0)
binstr=str-'0';

% get Hamming distance as number of base differences
% D = size(binstr,2) * pdist(binstr,'hamming');
ww=nthroot(2,10).^(0:eco_pars.ngenes-1);
w=reshape(repmat(ww,64,1),1,[]); % weight genes to have different mutation rates
w=w./mean(w);
binstr=binstr./w; % multiply bits by inverse mutation weights (less likely worth more)
D = pdist(binstr,'CityBlock')./size(binstr,2);

tic;tree = linkage(D);toc
